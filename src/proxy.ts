...
import { accountWithAuth, markSelected, requireAccount, type AccountView } from "./store.js";
...
export function defaultProxyState(): ProxyState {
  return {
    enabled: false,
    port: DEFAULT_PORT,
    token: "",
...
export function readProxyState(): ProxyState {
  return {
    ...defaultProxyState(),
    ...(readOptionalJson<Partial<ProxyState>>(getPaths().proxyStatePath) || {})
  };
}
...
export function selectProxyAccount(label: string): ProxyState {
...
function requestToken(req: IncomingMessage): string | null {
  ...
  const value = Array.isArray(header)
    ? header.find((entry): entry is string => typeof entry === "string")
...
function authorized(req: IncomingMessage, state: ProxyState): boolean {
...
function selectedAccount(state: ProxyState): { account: AccountView; accessToken: string; accountId: string } {
...
function stripRequestHeaders(req: IncomingMessage): Record<string, string> {
...
function stripUpgradeHeaders(req: IncomingMessage): Record<string, string> {
  const headers: Record<string, string> = {};
  for (const [key, value] of Object.entries(req.headers)) {
    const lowered = key.toLowerCase();
...
async function collectBody(req: IncomingMessage): Promise<Buffer | null> {
...
async function forward(req: IncomingMessage, res: ServerResponse, url: URL, state: ProxyState): Promise<void> {
...
export async function runProxyServer(port = readProxyState().port): Promise<void> {
...
export function probeSnapshot(snapshotPath: string): void {
