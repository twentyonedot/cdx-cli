...
function insertRootBlock(text, block) {
...
export function buildManagedBlock(port) {
    return [
        BEGIN,
...
export function enableCodexProxyConfig(port, existingBackupPath) {
    ...
    const backupPath = useExistingBackup ? existingBackupPath : `${configPath}.bak.cdx.${Date.now()}.${randomUUID()}`;
...
export function disableCodexProxyConfig(backupPath) {
    const configPath = getCodexConfigPath();
...
export function assertManagedConfigPresent() {
    const configPath = getCodexConfigPath();
